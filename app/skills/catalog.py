...
from app.skills.moderation import ModerationSkill
from app.skills.statistics import StatisticsSkill
from app.storage.repositories import LibraryRepository, ManagedChatRepository, MessageRepository, ModerationRepository, ScheduledPostRepository


def discover_skills(library_repository: LibraryRepository, management_adapter: ChatManagementAdapter, permissions, message_repository: MessageRepository, scheduled_posts: ScheduledPostRepository, managed_chats: ManagedChatRepository, moderation_repository: ModerationRepository) -> tuple[Skill, ...]:
    """Single skill catalog; the Telegram adapter does not know concrete skills."""
    return (StatisticsSkill(library_repository), LibrarySkill(library_repository), ManagementSkill(management_adapter, permissions), MessagesSkill(message_repository, permissions), SearchSkill(message_repository, permissions), BackupSkill(message_repository, management_adapter, permissions), SchedulerSkill(scheduled_posts, permissions), ModerationSkill(moderation_repository, managed_chats, management_adapter, permissions))
